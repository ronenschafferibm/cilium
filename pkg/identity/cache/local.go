...
// allocateNumericIdentity returns the next available numeric identity or an error
func (l *localIdentityCache) allocateNumericIdentity() (identity.NumericIdentity, error) {
	firstID := l.nextNumericIdentity
	for {
		if _, ok := l.identitiesByID[l.nextNumericIdentity]; !ok {
			id := l.nextNumericIdentity
...
func (l *localIdentityCache) lookupOrCreate(lbls labels.Labels) (*identity.Identity, bool, error) {
	...
	numericIdentity, err := l.allocateNumericIdentity()
	if err != nil {
		return nil, false, err
	}

	id := &identity.Identity{
		ID:             numericIdentity | identity.LocalIdentityFlag,
		Labels:         lbls,
...
func (l *localIdentityCache) release(id *identity.Identity) bool {
	...
	if id, ok := l.identitiesByLabels[stringRepresentation]; ok {
		...
		case id.ReferenceCount == 1:
			...
			delete(l.identitiesByID, (id.ID & ^identity.LocalIdentityFlag))
